...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-slate-200 bg-white/95 px-5 backdrop-blur-xs">
      {/* Left Title */}
      <div>
      ...
      <div className="relative flex items-center gap-2" ref={notifRef}>
        <Button
          ...
          className="relative h-8 w-8 rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900"
          title="Notifikasi"
        >
          <Bell size={18} />
        ...
        {openNotif && (
          <div className="absolute right-0 top-10 z-50 w-80 rounded-xl border border-slate-200 bg-white shadow-lg transition-all">
            ...
            <div className="max-h-64 overflow-y-auto divide-y divide-slate-100 p-1">
              {notifications.length === 0 ? (
                ...
                notifications.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-start gap-2.5 p-2.5 hover:bg-slate-50/80 rounded-lg transition-colors"
                  >
